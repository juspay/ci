...
import System.IO (BufferMode (LineBuffering), Handle, IOMode (WriteMode), hClose, hIsEOF, hPutStrLn, hSetBuffering, withFile)
...
runScript sinks name mLog script =
  bracket createPipe closeBoth $ \(rd, wr) -> do
    ...
    withCreateProcess cp $ \_ _ _ ph -> do
      ...
      -- the 'UseHandle' contract); we still need 'rd' open here to drain.
      hSetBuffering rd LineBuffering
      drainStream sinks name mLog rd `finally` hClose rd
      waitForProcess ph
  where
    -- Bracket cleanup if we exit before withCreateProcess closes 'wr'.
    closeBoth (rd, wr) = hClose rd `safe` hClose wr
    safe a b = a `finally` b
    ...
    loop = do
      ...
      if eof
        then pure ()
          ...
          line <- TIO.hGetLine h
          for_ mLog $ \lh -> hPutStrLn lh (T.unpack line)
